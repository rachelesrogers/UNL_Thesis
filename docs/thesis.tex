...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{comments }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"data/clean\_notes.csv"}\NormalTok{)}
\CommentTok{\# length(unique(subset(comments, notes !=}
\CommentTok{\# \textquotesingle{}\textquotesingle{})$clean\_prints)) val\_prints \textless{}{-}}
\CommentTok{\# sample(unique(subset(comments, notes !=}
\CommentTok{\# \textquotesingle{}\textquotesingle{})$clean\_prints), size=30, replace=FALSE)}
\CommentTok{\# val\_prints \textless{}{-} c(val\_prints, 503, 359, 39,}
\CommentTok{\# 368, 547) val\_dataset \textless{}{-} comments \%\textgreater{}\%}
\CommentTok{\# subset(clean\_prints \%in\% val\_prints)}
\CommentTok{\# write.csv(val\_dataset,}
\CommentTok{\# \textquotesingle{}validation\_dataset.csv\textquotesingle{}) dim(val\_dataset)}

\NormalTok{test\_comments }\OtherTok{\textless{}{-}}\NormalTok{ comments[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{, ]}

\FunctionTok{kable}\NormalTok{(test\_comments }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{filter}\NormalTok{(page\_count }\SpecialCharTok{==} \DecValTok{2} \SpecialCharTok{|}\NormalTok{ page\_count }\SpecialCharTok{==} \DecValTok{3}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(page\_count, notes), }\AttributeTok{caption =} \StringTok{"Participant Note Example"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
...
\caption{\label{tab:unnamed-chunk-35}Participant Note Example}
\centering
...
This version of the thesis was generated on 2023-05-15 11:26:46.
