...
In 1923, }\emph{Frye vs.~United States} \authorcol{established a standard of 'general acceptance' in the scientific community for the admissibility of scientific evidence} (NRC, 2009, p. 88).
\authorcol{In 1975, the Federal Rule of Evidence 702 instead promoted "...mere 'assistance' to the trier of fact.." as the basis of admissibility, although whether} \emph{Frye} \authorcol{or Rule 702 should be used for admissibility was not clarified until } \emph{Daubert v. Merrell Dow Pharmaceuticals, Inc.}, \authorcol{which sided with Rule 702, with the added caveat that 'evidentiary reliability' should be established, although some states still use the} \emph{Frye} \authorcol{standard} (NRC, 2009, pp. 89--91).
\authorcol{PCAST clarifies that: "The expert's testimony must be grounded in an accepted body of learning or experience in the expert's field, and the expert  must explain how the conclusion is so grounded"} (NRC, 2009, p. 94).
\authorcol{However, this standard of evidence often does not lead to inadmissible determinations, as demostrated in} \emph{United States v. Green}, \authorcol{where Judge Green stated that, while toolmark identification should not be admissible in terms of the} \emph{Daubert} \authorcol{ruling, it would be difficult to exclude due to precedent; he did, however, limit the examiner's conclusion language such that the examiner could not testify to a 'definitive match'} (NRC, 2009, p. 108).

\hypertarget{language-in-the-courtroom}{%
\subsection{Language in the Courtroom}\label{language-in-the-courtroom}}

These differing views \authorcol{amongst individuals in the field} are represented in Swofford \& Champod (2022).
...
Garrett \& Mitchell (2013) suggests that this lack of difference may relate to the overall view of firearms evidence as reliable, meaning that even a weak match simply boils down to being a match in the eyes of the participants (p.~507).
...
United States Courts for the Ninth Circuit (2019) also suggest to avoid labeling such testimony as expert testimony \authorcol{altogether}, since it may cause jurors to give the testimony undue weight.
...
Garrett \& Rudin (2023) state that ``\ldots interpretability is particularly important in legal settings, where human users of a system\ldots{} cannot fairly and accurately use what they cannot understand.''.
...
The three laboratory managers interviewed by Swofford \& Champod (2022) suggested using algorithms as a supplement to the forensic examiner (p.~6), similar to Montani et al. (2019)'s suggestion.
...
\authorcol{In many forensic science papers, there are two approaches that are commonly defined as using a "likelihood ratio" approach, although one is truly a Bayes Factor approach} (Ommen \& Saunders, 2021).
\authorcol{According to }Ommen \& Saunders (2021). \authorcol{the usual form of "likelihood ratios" in forensic science is written as} \(\frac{P(E|H_p)}{P(E|H_d)}\), \authorcol{where E is the evidence, $H_p$ is the prosecution's hypothesis, and $H_d$ is the defense's hypothesis; however, there are the methods differ in how $P$ is calculated (pp. 348 - 349).}
Ommen \& Saunders (2021) \authorcol{states that "...the Bayes Factor is the expected value of the likelihood ratio function with respect to the posterior distribution of the parameters given the entire collection of evidence generated under the defense model." (p. 351).
...
Marquis et al. (2016) suggest that the use of a likelihood ratio requires examiners to consider what they are including when weighing the strength of evidence, and provides a value that can be consistent across disciplines (p.~4).
...
A similar scale is also suggested by Evett (1998), with verbal values of ``Limited support'', ``Moderate support'', ``Strong support'', and ``Very strong support'' (p.~201).
...
\authorcol{While the overall ideas, or constructs, that we wish to measure remain the same, different response types may provide more or less useful mappings to the concept to be measured} (Groves et al., 2009).
...
\authorcol{Due to the issues outlined in Chapter} \ref{litreview}, \authorcol{we designed a study to investigate how potential jurors may perceive a bullet matching algorithm, when presented alongside a firearms examiner's testimony, as well as the affect of demonstrative evidence.
...
The foundational belief in bullet comparisons as a form of identifying evidence \authorcol{revolves around the idea} that guns can leave unique striation marks on a bullet as an artifact of the rifling process (PCAST, 2016).
...
\authorcol{Because the images are intended to aid only in interpretation, unbiased results would include an increase in understanding across all other conditions, while reliability and credibilty stay the same.}
...
\authorcol{They were first given information about their responsibility as jurors to choose whether or not to convict, based on the 'beyond reasonable doubt' threshold.}
...
The study includes three \authorcol{independent} variables: \authorcol{presence or absence} of the algorithm, \authorcol{presence or absence} of demonstrative evidence (images), and conclusion (identification, elimination, or inconclusive).
...
\authorcol{According to Prolific, these participants are a representative sample (in terms of age, sex and ethnicity, based on census data) of Prolific participants located in the United States.}
...
These 569 participants \authorcol{who answered both attention check questions correctly} were considered for the following analysis.
...
\authorcol{Many of these response scales suffered from scale compression - individuals selected the top two categories on many of the Likert scales, indicating that participants overall believed in the credibility of the witnesses and the reliability and scientificity of the methods.
...
10 out of 196 individuals in the elimination category, 13 out of 191 individuals in the inconclusive category, and 112 out of 182 individuals in the identification category chose to convict, despite \authorcol{the fact that the bullet comparison testimony was the only evidence presented.}
...
As Figures \ref{fig:cred} and \ref{fig:algcred} demonstrates, ``Extremely credible'' was by far the most selected category for the firearms examiner and the algorithm expert, with some people selecting ``Moderately credible'', while the other categories quickly drop off in responses.
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.4306}}
...
\authorcol{Unlike questions of reliability, credibility, and scientificity, strength of evidence was measured on a 9-point Likert scale, without worded values for intermittent categories, following the procedure of} Garrett, Scurich, et al. (2020).
...
The testimony transcript format \authorcol{used in the initial study} did not clearly identify the speaker with each line, instead using ``Q:'' and ``A:'' in most cases, as shown in Appendix \ref{study-2-changes}.
...
{\centering \includegraphics[width=\linewidth]{images/Study2_Screenshot} 
...
In the course of this investigation, we \authorcol{discovered two aspects of the testimony that we wished to update: speaker clarity and response type.}
\authorcol{While the initial study responses suffered from scale compression, the participants' notepads acted as another source of information.}
...
We investigated various response types, \authorcol{using a variety of methods, from betting to numeric chance scales.}

\authorcol{The text analysis provided useful information on participant focus, especially when evaluating the testimony that was added in the response study. 
...
\authorcol{The new study format was received favorably, based on participant comments.
This survey format is designed to be made available to a wider audience for conducting studies of trial scenarios.
...
This version of the thesis was generated on 2024-03-11 16:22:44.021103.
...
\leavevmode\vadjust pre{\hypertarget{ref-ommenProblemForensicScience2021}{}}%
