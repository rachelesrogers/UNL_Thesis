...
For participants from the United States and Canada, 38 of 491 comparisons were reported as inconclusive for known matches, while 254 of 693 comparisons were reported as inconclusive for know\authorcol{n} non-matches (Chapnick et al., 2021).
...
In general, individuals want to guarantee that the information presented in the courtroom is \authorcol{correct} and valid, without causing too much imbalance between the limitations and the categorical conclusion.
...
According to Ommen \& Saunders (2021), the usual form of ``likelihood ratios'' in forensic science are written as \(\frac{P(E|H_p)}{P(E|H_d)}\), where E is the evidence, \(H_p\) is the prosecution's hypothesis, and \(H_d\) is the defense's hypothesis; however, there are \authorcol{two different} methods \authorcol{for calculating the probability,} \(P\)\authorcol{: either as a Bayes Factor or as a frequentist likelihood ratio}.
...
Thompson, Kaasa, \& Peterson (2013) asked participants \authorcol{to} rate the chance of guilt with values generally on a log 10 scale (ex. 1 in 100; 1 in 1,000); a middle value of a fifty-fifty chance; and extreme values of ``Certain to be guilty'' and ``Impossible that he is guilty''.
...
We selected options to recruit a representative sample\authorcol{, based on age, sex, and gender,} of individuals located in the United States, and asked that participants self-screen for jury eligibility before completing the survey.
...
Figure \ref{fig:completiontime} depicts completion times (from completion of the demographics information to the end of the survey) by conditions for unique fingerprints, or identification values (n=\authorcol{539}), excluding 3 observations greater than 75 minutes.
\authorcol{In several cases, there was repetition of electronic fingerprints, which were meant to serve as unique identifiers in the dataset.
...
\authorcol{Table} \ref{tab:timetab} \authorcol{shows the ANOVA table of completion times, by experimental condition.}
...
This concept \authorcol{w}as later extended to include more extensive strings, as demonstrated by Konstantinidis (2005).
...
However, there is not a significant difference between studies (\authorcol{Pearson's chi-squared} p-value 0.43).
...
\authorcol{There is no significant difference between the hidden and visible probability scales, based on a paired t-test (p-value 0.73).}
...
\authorcol{In the future, we may also aim to establish test-retest reliability for these response types.
...
This version of the thesis was generated on 2024-04-17 14:45:27.228138.
