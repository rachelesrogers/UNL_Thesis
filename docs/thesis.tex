...
When a person's future hangs in the balance of such evidence, we must know that the type of evidence being presented is something that can be relied upon, and that the jury can weigh it appropriately when reaching a conclusion.
...
By testing jury perception in bullet matching, and by developing tools for assessing jury perception, we hope to expand what can be learned about jury perception in order to present statistical methods in a way that can be understood and used to make decisions by the wider public.
...
Biasotti (1959) suggests that statistical methods can be used in order to determine whether or not two bullets were fired from the same gun.
...
Figure \ref{fig:microscope} depicts a comparison microscope, along with aligned striation marks between two bullets.
...
Some issues outlined by PCAST (2016) are the circular nature of AFTE's identification guidelines and the lack of appropriately designed error rate studies {[}104{]}.
...
Following PCAST's publication, many methods of statistical matching have been developed and evaluated, such as (Hare et al., 2017; J. Song et al., 2012; Vanderplas et al., 2020; Vorburger, Song, \& Petraco, 2015).
...
Participants are presented with a short scenario based on Garrett, Scurich, \& Crozier (2020): a bullet is the only evidence recovered from an attempted convenience store robbery, and is tested against a gun found in Richard Cole's car in a routine traffic stop.
...
Participants were compensated with \$8.40 for completing the study, for an hourly compensation rate of about \$27.79 (average completion time of approximately 18 minutes).\\
...
For reliability, show in Figure \ref{fig:histrel}, a similar number of individuals selected ``Extremely reliable'' for both the algorithm evidence and the examiner's comparison, with a larger proportion of individuals selecting ``Extremely reliable'' in the match condition when the algorithm is present.
...
In order to clarify the speakers in the testimony and create a more engaging environment, we commissioned character drawings from Richy Meleus to represent various actors that may be present in the courtroom. These characters' faces and clothing are largely exchangeable, allowing for a large combination of clothing and characteristics.
...
This version of the thesis was generated on 2023-04-18 12:08:22.
