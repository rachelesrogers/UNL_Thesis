...
\textbackslash svp\{even though Bielow et al. (2016) warns that this LCS method is inefficient, and meant to be used in small instances.
...
\bottomrule\noalign{}
\endlastfoot
2 & discharge firearm in business-
felony, not guilty \\
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.3151}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.0959}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.1096}}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.3026}}
...
\svp{I think identifying the root cause here: participants couldn't match text to the "side" of the adversarial process, and then providing examples of how that was addressed (with pictures) is important for clarity. Use fewer "may" statements and instead support your claims with participant comments, if possible.}
...
The lack of visual cues for \svp{matching text with the speaker} is not representative of the courtroom setting, and may have contributed to the confusion expressed by some participants.
...
Appendix \ref{study-2-changes} shows the characters that have been developed for this study (drawn by Richy Meleus), as well as a screenshot of the study format. \svp{I would put the characters and the screenshot in the text -- they're important}
...
To investigate a which response type may be most informative for this study and to troubleshoot the use of figures and speech bubble format, we conducted a micro study using various response types. \svp{I think you need to link this back to scale compression and explicitly review the literature corresponding to these different response types.}
...
In terms of conviction, participants were reminded of the decision criterion of ``beyond a reasonable doubt'' when making a decision in a criminal trial, and asked if they would choose to convict. \svp{Show screenshots of each scale in a figure here, please}
...
In both cases, the numerator had to be less than or equal to the denominator, \textbackslash svp\{restricting the numerical result to the \([0,1]\) range.
...
There were 3 responses with values greater than 1 that will not be considered in this analysis.\svp{What failed here to allow participants to respond $> 1$?}
...
This resulted in a significant difference between the two groups (p-value of 0.006).\svp{Mention how this p-value came to be - what test did you use?}
...
Those who chose to express numeric choice in terms of innocence were less likely to input a value corresponding to their multiple choice selection (45.24\% consistent, or 76 out of 168 consistent for chance of innocence, and 66.15\% consistent, or 86 out of 130 consistent for chance of guilt).
This represents a significant difference in consistent ranges based on how participants chose to express the numeric chance (p-value: \ensuremath{5\times 10^{-4}}).
...
This may be a result of confusion in the scale reversal of the chance of innocence scale, while the chance of guilt smoothly\svp{wording?? I get what you're going for, but smoothly reads a bit strange here} corresponds with expressing the probability of guilt.
...
This version of the thesis was generated on 2024-02-29 14:29:37.550802.
