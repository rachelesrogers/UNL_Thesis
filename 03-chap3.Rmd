...
According to @grovesSurveyMethodology2009, an error in survey measurements may result from a difference between the construct that we wish to evaluate, and the measurement used to evaluate it.
...
@grovesSurveyMethodology2009 uses the National Assessment of Educational Progress as an example of the difference between construct and measures: "when measuring the construct of mathematical abilities of 4th graders, the measures are sets of arithmetic problems" (50).
...
In investigating  scale compression, we found that @OpinionEvidence suggests that the use of the term "expert" may result in jurors giving the testimony more weight, and suggests judges use the following jury instruction:
...
>"...Where's the defense in all of this? It would have been  helpful to know whether or not the defendant had an alibi, whether his firearm had in fact been recently fired, from a forensics perspective, whether there were any in-store or exterior or neighborhood cameras that picked up whoever discharged the firearm. Convenience stores nearly always have a height chart posted alongside the door but no one provided an approximate height or weight for the person who discharged the firearm... " - Inconclusive, No Algorithm, Demonstrative Evidence
...
include_graphics("images/study_questions1.jpg")
...
include_graphics("images/study_questions2.jpg")
...
However, there is not a significant difference between studies (\authorcol{Pearson's chi-squared} p-value `r round(match_convict_compare$p.value, 2)`).
...
# prob_comparison <- microstudy_clean %>%
#   select(conclusion, visible_probability, hidden_probability) %>%
#   mutate(hidden_probability = as.numeric(hidden_probability)) %>%
#   pivot_longer(!conclusion, names_to="probability_type", values_to="value")
# 
# hid_vis_test <- lm(value ~ conclusion*probability_type, data = prob_comparison)
# Anova(hid_vis_test, type = "III") %>% pander(style = "grid",
#          caption = "(\\#tab:hidvisprob) Type III ANOVA Table for Probability Values", justify = 'left')

prob_compare<-t.test(microstudy_clean$visible_probability, as.numeric(microstudy_clean$hidden_probability), paired=TRUE, alternative="two.sided")

```

Figure \@ref(fig:prob2) shows participant-selected probabilities that the defendant committed the crime.
For a single participant, their \authorcol{selection of} hidden probability was recorded incorrectly (carrying over a previous answer), I have removed this observation when performing analysis on the hidden probability scale.
...
\authorcol{There is no significant difference between the hidden and visible probability scales, based on a paired t-test (p-value `r round(prob_compare$p.value, 2)`).}
...
As shown in Figure \@ref(fig:opinionchanceplot), those who thought the defendant was innocent were more likely to favor inputting chance values in terms of innocence(`r round(innocent_chance[1]/opinion_totals[1]*100, 2)`\%, or `r innocent_chance[1]` out of `r opinion_totals[1]`), while those who thought the defendant was guilty were slightly less likely to express chance in terms of innocence (`r round(innocent_chance[2]/opinion_totals[2]*100, 2)`\% expressed their beliefs in terms of innocence, or `r innocent_chance[2]` out of `r opinion_totals[2]`).
This resulted in a significant difference between the two groups (Pearson's chi-squared p-value of `r round(response_choice$p.value, 3)`).
...
@schwarzSAGEHandbookMeasurement2010 indicate that order matters in partially-redundant questions, where participants may assume that later questions are asking about different facets than the initial question.
...
```{r}
#| questorder,
#| fig.cap= "Order of Study Questions",
#| fig.width= 12,
#| fig.height= 5,
#| fig.align= "center",
#| echo= FALSE,
...
microstudy_response_order <- read_csv("data/microstudy_response_order.csv")

microstudy_response_order <- microstudy_response_order %>%
  group_by(randomnumber, start_time) %>%
  arrange(page) %>%
  mutate(sequential_page = 1:n())

# table(microstudy_response_order$page, microstudy_response_order$sequential_page)

microstudy_response_order %>%
  subset(!(question %in% c("convict", "def_comment", "guilt_opinion", "comments"))) %>%
  ggplot(aes(x=question, fill=as.factor(sequential_page)))+
  geom_bar() + labs(fill="Page")+
  scale_fill_brewer()+
  theme_bw()
...
bet_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="betting",]$sequential_page))

vis_prob_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="visible_probability",]$sequential_page))

hid_prob_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="hidden_probability",]$sequential_page))

fixed_chance_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="chances_fixed",]$sequential_page))

check_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="check",]$sequential_page))

strength_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="evidence_strength",]$sequential_page))

num_chance_test <- 
  chisq.test(table(microstudy_response_order[microstudy_response_order$question=="numeric_chance",]$sequential_page))

order_test_df <- data.frame(Question = 
                              c("Betting", "Hidden Probability", 
                                "Visible Probability", "Fixed Chance", "Check", 
                                "Strength", "Numeric Chance"),
                            Pvalue = c(bet_test$p.value, hid_prob_test$p.value,
                                       vis_prob_test$p.value,
                                        fixed_chance_test$p.value, check_test$p.value,
                                        strength_test$p.value, 
                                       num_chance_test$p.value))

order_test_df %>%  pander(style = "grid",
         caption = "(\\#tab:ordertest) Chi-Squared Tests for Question Order", justify = 'left')
...
microstudy_pcp %>%
  ggplot(aes(vars=vars(c(6,5,1,4)))) +
  geom_pcp(aes(colour = Conclusion), boxwidth=0.6, alpha=0.9) +
  geom_pcp_box(boxwidth=0.6, fill=NA) +
...
ggplot(microstudy_clean, aes(x=betting,
  ...
  #                 aes(size = log(after_stat(density))), n = 20, contour = FALSE)+
  # stat_density_2d(geom = "raster", aes(fill = log(after_stat(density))), contour = FALSE) +
  # geom_point()+
  # geom_density_2d(linewidth = 0.25, colour = "black")+
  # stat_density_2d(aes(fill = log10(..density..)), geom = "raster", contour = FALSE, linewidth = 0.25, colour = "black") +
...
Whether the participants were betting on the guilt or innocence of the defendant depended on their personal opinion of guilt - those who thought the defendant was guilty bet on the defendant's guilt, while those who thought the defendant was innocenc\authorcol{t} bet on the defendant's innocence.
...
#### Strength and Probability Comparison
...
Base\authorcol{d} on Figure \@ref(fig:convictsincome), some individuals in the match condition chose to convict across all income levels.
...
# freq_summary <- as.data.frame(table(responsetype_unclean_notes$randomnumber))
# shorter_rand <- freq_summary[freq_summary$Freq<14,]$Var1

# table(freq_summary$Freq)
# 
# View(responsetype_unclean_notes[responsetype_unclean_notes$randomnumber %in% shorter_rand,])
# 
# has_pg18 <- responsetype_unclean_notes[responsetype_unclean_notes$page==18,]$randomnumber
# 
# has_pg4 <- responsetype_unclean_notes[responsetype_unclean_notes$page==4,]$randomnumber
# 
# has_both <- has_pg4[has_pg4 %in% has_pg18]
# 
# table(responsetype_unclean_notes[responsetype_unclean_notes$randomnumber %in% has_pg4,]$page)
# 
# ggplot(filter(responsetype_unclean_notes,randomnumber %in% has_both), aes(group=randomnumber, y=time, x=page,
#                                        color=randomnumber))+
#   geom_line()+
#   geom_point()
...
# responsetype_clean_notes$updated_page <- ifelse(responsetype_clean_notes$randomnumber %in% has_pg4, responsetype_clean_notes$page, responsetype_clean_notes$page - 1)
# View(responsetype_clean_notes)
# 
# responsetype_clean_notes[responsetype_clean_notes$randomnumber %in% has_both &
#                            responsetype_clean_notes$page==18,]$updated_page <- 17
# 
# table(responsetype_clean_notes$updated_page)

# write.csv(responsetype_clean_notes, "data/microstudy_hybrid_pageupdate.csv")

responsetype_clean_notes <- read.csv("data/microstudy_hybrid_pageupdate.csv")

# responsetype_clean_notes[responsetype_clean_notes$randomnumber %in% has_pg4,]
...
A few of the participants wrote the firearms examiner's answer in their notepad, indicating that they paid attention to this question, \authorcol{specifically in the identification condition}.
...
>"The visuals of the conversation really helped me to focus in on the study. Also GREAT idea with the notepad, i tend to forget important information when im overloaded and it helps to be able to jot things down..." - Identification
