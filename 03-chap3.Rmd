...
The second question allowed participants to select both the numerator and denominator: "About ___ chance(s) in ___ that the defendant is (innocent/guilty)", where participants were able to select to express the value as innocence or guilt.
...
microstudy$gender = factor(microstudy$gender,
...
Of the `r dim(microstudy)[1]` completed surveys, `r sum(microstudy$gender=="Man")` participants were men, `r sum(microstudy$gender=="Woman")` participants were women, and `r sum(microstudy$gender=="Other/non-binary")` participants identified as other or non-binary. 
...
`r dim(microstudy_clean)[1]` participants passed the attention check, meaning that only `r sum(microstudy$check!="9mm", na.rm=TRUE) + sum(is.na(microstudy$check))` participants failed the attention check, and will not be included in the analysis.
...
ggplot(microstudy_clean) +
  geom_bar(aes(x=convict, fill=guilt_opinion), position="dodge") +
  ggtitle("") +
...
ggplot(microstudy_clean) +
  geom_histogram(aes(x=betting, fill=guilt_opinion), binwidth=5, color="black",
...
ggplot(microstudy_clean) +
  geom_bar(aes(x=chances_fixed, fill=conclusion), position="dodge") +
...
<!-- , so that the orders can be compared. -->
...
The lines indicate the numerical equivalent to the multiple choice wording.
...
ggplot(subset(clean_results_merged, chance_of=="guilty" & 
              chances_fixed %in% c("Impossible that he is guilty", 
                                   ...
                                   "About 1 chance in 1,000",
                                   "About 1 chance in 100",
                                   "About 1 chance in 10")), aes(x=num_chance, fill=chances_fixed))+
  ggtitle("Chances of Guilt") +
  ylab("Count")+
...
ggplot(subset(clean_results_merged, chance_of=="guilty" & 
              chances_fixed %in% c("Certain to be guilty", 
                                   ...
                                   "About 999 chances in 1,000",
                                   "About 99 chances in 100",
                                   "About 9 chances in 10")), aes(x=1-num_chance, fill=chances_fixed))+
  ...
  theme_bw()+
   scale_x_log10()+
  geom_vline(aes(xintercept=1/10000), color="#FE6100", size=1)+
  geom_vline(aes(xintercept=1/1000), color="#DC267F", size=1)+
  geom_vline(aes(xintercept=1/100), color="#785EF0", size=1)+
  geom_vline(aes(xintercept=1/10), color="#648FFF", size=1)
...
The only multiple choice value that shows some consistency with the numeric counterpoints is "About 1 chance in 10", while for all other multiple choice options participants tended to provide a lower chance of numerical innocence than their multiple choice selection would indicate.
...
ggplot(subset(clean_results_merged, chance_of=="innocent" & 
              chances_fixed %in% c("Impossible that he is guilty", 
                                   ...
                                   "About 1 chance in 100",
                                   "About 1 chance in 10")),
       aes(x=1-num_chance, fill=chances_fixed))+
  ggtitle("Chances of Guilt") +
  ylab("Count")+
...
Although this consistency is not as present in the other categories, they are closer to the ballpark than what is shown in Figure \@ref(fig:likecomp2scale).
...
ggplot(subset(clean_results_merged, chance_of=="innocent" & 
              chances_fixed %in% c("Certain to be guilty", 
                                   ...
                                   "About 999 chances in 1,000",
                                   "About 99 chances in 100",
                                   "About 9 chances in 10")), aes(x=num_chance, fill=chances_fixed))+
  ggtitle("Chances of Innocence") +
  ylab("Count")+
...
\authorcol{An issue with the use of the log scale is the presence of 0's, when the scale is not reversed, and 1's when the scale is reversed.}
There was a single individual who chose a chance of 0: this person thought that the chance of the defendant committing the crime was 0, and chose "Impossible that he is guilty", demonstrating alignment in chance choices.
There were `r sum(clean_results_merged$num_chance==1)` individuals, on the other hand, who chose a chance value of 1.
These individuals, along with their multiple choice chance decisions, are depicted in Figure \@ref(fig:onevaluesmosaic).
For a numerical chance value of 1, the corresponding multiple choice value would be "Impossible that he is guilty" when expressing the chance of innocence, and "Certain to be guilty" when expressing the chance of guilt.
These were the most populated selections on the scale.
Two individuals selected that the defendant was "Certain to be guilty" or had "About 9,999 chances in 10,000" of being guilty while indicating that the chance of innocence was 1 - showing a potential reversal of the scale when participants express their numerical conclusions in terms of innocence.
Approximately half of the individuals who chose to indicate that the numeric chance was 1 did not select the corresponding extreme value in the multiple choice scale, and instead chose a value somewhere in the middle.
...
ggplot(clean_results_merged, aes(x=chances_fixed))+ #,fill=conclusion
  ...
  geom_jitter(aes(y=(visible_probability/100), color=guilt_opinion),
  ...
  geom_boxplot(aes(y=(visible_probability/100)),
               position = position_dodge(1),
               alpha = 0.5,
               outlier.shape = NA)+ 
