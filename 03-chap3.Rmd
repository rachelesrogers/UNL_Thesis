...
This resulted in a significant difference between the two groups (\authorcol{Pearson's chi-squared test statistic} p-value of `r round(response_choice$p.value, 3)`).
...
I performed Chi-Square tests to test for equality across categories, and the p-values from these tests are shown in Table} \@ref(tab:ordertest)\authorcol{, which indicates no significant difference between page number for any of the question categories.}
