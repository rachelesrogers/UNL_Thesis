...
```{r shoulder, echo=FALSE, fig.cap="The land is shown in the center. Shoulders are shown outside of the vertical lines. Hare et al. (2017)", fig.scap="The land is shown in the center"}
...
```{r signature, echo=FALSE, fig.cap="Two aligned signatures for a known match. Image generated from Houston dataset by authors.", fig.scap="Two aligned signatures for a known match"}
...
```{r grid, echo=FALSE, fig.cap="Diagonal correspondence (in orange) among lands indicate a match, as shown here. Image generated from Houston dataset by authors.", fig.scap="Diagonal correspondence (in orange) among lands indicate a match."}
...
demo_fingerprint <- table(demographics$clean_prints)
demo_df <- data.frame(demo_fingerprint)
demo_single <- demo_df %>% filter(Freq==1)

demo_sub <- demographics %>% filter(clean_prints %in% demo_single$Var1) %>%
  dplyr::rename(start_time = time)

beginning_wend <- inner_join(demo_sub, scenarios)
beginning_wend$total_time <- NA
beginning_wend$total_time <- beginning_wend$finish_time - beginning_wend$start_time
...
dim_l_0 <- beginning_wend %>% filter(total_time_minutes < 0) %>% dim()
...
time.fit <- lm(total_time_minutes ~ algorithm*conclusion*picture, data = beginning_wend)
Anova(time.fit, type = "III") %>% pander(style = "grid",
         caption = "(\\#tab:timetab) Type III ANOVA Table for Completion Times", justify = 'left')

```

Figure \@ref(fig:completiontime) depicts completion times (from completion of the demographics information to the end of the survey) by conditions for unique fingerprints, or identification values (n=`r dim(beginning_wend)[1]`), excluding `r dim_g_75[1]` observations greater than 75 minutes. 
...
Those who received the algorithm condition took more time on average than those who did not (median values of `r  round(median(beginning_wend[beginning_wend$algorithm=="Yes",]$total_time_minutes, na.rm=TRUE), digits=3)` minutes and `r round(median(beginning_wend[beginning_wend$algorithm=="No",]$total_time_minutes, na.rm=TRUE), digits=3)` minutes, respectively). 
...
#| fig.scap= "Probability the gun was used in the crime, or that Cole committed the crime.",
#| fig.width= 10,
#| fig.height= 6,
#| fig.align= "center",
#| echo= FALSE,
