...
When conducting a bullet comparison, it is assumed that guns can be uniquely identified, based on individualizing marks left on a bullet when fired from the gun, due to random variation in barrel rifling and the manufacture process [@pcast, p. 104]. 
...
The use of an \chaircol{automatic,} objective method would contribute to quantifiable presentation of evidence \chaircol{and would lower the effort required to identify the method's error rate}. 

### Bullet Matching Algorithm

Following PCAST's publication, many methods of statistical matching have been developed and evaluated, such as [@hare2017automatic;@vanderplasComparisonThreeSimilarity2020;@songDevelopmentBallisticsIdentification2012;@vorburgerTopographyMeasurementsApplications2015]. 
...
1. A 3D scan is taken of each bullet land (containing striation marks to be compared), a stable cross-section is extracted, and shoulders (without relevant striation marks) are removed\chaircol{, as in} \@ref(fig:shoulder), \authorcol{an image from} @hare2017automatic.
...
The signature can then be compared to land signatures from other bullets \@ref(fig:signature).

```{r signature, echo=FALSE, fig.cap="Two aligned signatures for a known match. Image generated by authors from Houston dataset."}
include_graphics(path = "images/F526_Match_Signatures.png")
```

3. Traits, such as consecutively matching striae and depth of grooves, are then combined and a random forest is used to produce a match score (ranging from 0 to 1) for the lands. 
...
Lands are then aligned across bullets in order to compute an overall match score for the bullets \@ref(fig:grid).

```{r grid, fig.cap="Diagonal correspondence (in orange) among lands indicate a match, as shown here. Image generated by authors from Houston dataset."}
include_graphics(path = "images/Test_Fire_F526.jpeg")
```

This algorithm was trained on \chaircol{we need to specify which sets here, because there are a *ton* of hamby sets...} Hamby's Consecutively Rifled Ruger Barrel Study data set\authorcol{s 252 and 173}[@vanderplasComparisonThreeSimilarity2020], where the final random forest was able to correctly predict all matches and non-matches [@hare2017automatic, p. 2350]. 
...
This scale ranged from light support, corresponding to a likelihood ratio between 1 and 10, to very strong support, corresponding to a likelihood ratio greater than 10,000 [@ENFSIGuidelineEvaluative2016, p. 64]. 
...
Hare et al.'s algorithm differs from previous presentation methods in that its output is a match score, as opposed to a likelihood ratio. 
In this study, potential jurors may encounter both the algorithm's match score, as well as the categorical match language of the examiner.

### Demonstrative Evidence

Images are often used to assist individuals in understanding non-image information. 
However, images may have an effect of making a scenario more believable, as demonstrated \chaircol{in a study by} @cardwellNonprobativePhotosRapidly2016 \chaircol{which involved} "giving" or "taking" food from animals with or without images. 
...
Therefore, the use of images in a courtroom should be studied for a compounding effect with regards to how subjects perceive the evidence presented - namely, if there is a difference in how reliable or credible they feel the experts are.

## Methods

### Study Format

Participants are presented with a short scenario based on @garrettMockJurorsEvaluation2020: A bullet is the only evidence recovered from an attempted convenience store robbery, and is tested against a gun found in Richard Cole's car in a routine traffic stop. 
...
The testimony was based on actual court testimony provided by forensics experts lawyers, and judges. \chaircol{XXX Add appendix with citations to specific trial transcripts} \authorcol{added to Thesis Document}
...
The study includes three variables: use of the algorithm, use of demonstrative evidence (images), and conclusion (match, exclusion, or inconclusive). 
...
By presenting the algorithm results with the interpretation suggested by the firearms expert, we hypothesized that potential jurors could use the firearms expert's explanation and conclusion to guide their understanding of the algorithm's results. 
...
Those who received the algorithm were asked about the reliability of the forensics examiner's bullet comparison, the reliability of the algorithm's comparison, the overall reliability in the case (which includes both the algorithm and the forensics expert), as well as the reliability of the field of firearms as a whole. 
...
Participants were compensated with \$8.40 for completing the study, \chaircol{for an hourly compensation rate of about} \$27.79 (average completion time of approximately 18 minutes).  
\authorcol {Individuals who did not include their Prolific identification number and an individual whose notes indicated that they had progressed far enough into the survey to get a separate scenario before restarting were excluded from analysis.}
\chaircol{For your thesis, include a chart of completion time distribution, facetted by condition}
\@ref(fig:completiontime) \authorcol{depicts completion times (from completion of the demographics information to the end of the survey) by conditions for unique fingerprints(n=559), excluding five observations greater than 75 minutes. In general, it appears that those who received the algorithm condition took slightly more time on average than those who did not (median values of 19.13 minutes and 13.72 minutes, respectively). This is unsurprising, given the increased length of the algorithm condition.}
...
#beginning_wend %>% filter(total_time_minutes > 75) %>% dim()
...
Of the \authorcol{636} participants with Prolific ID's who started the survey, there were \authorcol{324} female participants, \authorcol{307} male participants, and \authorcol{5} non-binary participants.
The median age was \authorcol{45}. 
Age and gender are shown in \@ref(fig:demographics).
\authorcol{An age of 0.2 age was excluded due to implausability and lack of study completion.}
\authorcol{591} participants \authorcol{with a unique Prolific ID and without significant notes from a previous run through} completed the survey, and \authorcol{569} participants correctly answered both attention check questions.
The division of correct answers on the attention check questions are shown in \@ref(tab:attentioncheck).
This is the number of participants who were considered for the following analysis.
\chaircol{Charts may be useful here, again, only for your thesis and not for the actual publication - visuals are helpful but if space is limited then these types of visuals are the first to go to an appendix.}
...
Another question asked participants if they thought Cole was guilty of the crime. 
...
There is a difference in the perceived probability that Cole committed the crime as well as the perceived probability that the gun was present at the crime scene based on the examiner's decision, as shown in \@ref(fig:probalgorithm). 
The match condition resulted in high probabilities, while the non-match condition resulted in low probabilities, \chaircol{indicating that participants reacted to the examiner's testimony}. 
...
The distribution of probabilities based on conclusion is shown in \@ref(fig:probalgorithm) \chaircol{XXX ???}. 
...
Because the match score is on a scale of 0 - 1, this value could be misinterpreted as a probability for the gun being used in the crime. 
It does not appear that the participants are anchoring to this value, however, as the distributions do not appear to correspond more to the line when the algorithm is present. \chaircol{Let's choose contrasting colors here, if possible - red/blue, so the overlap is purple? }

```{r}
#| probalgorithm,
#| fig.cap= "Probability the gun was used in the crime, or that Cole committed the crime. Black lines indicate values produced by the algorithm",
#| fig.width= 10,
#| fig.height= 6,
#| fig.align= "center",
#| echo= FALSE,
...
\authorcol{10/196} individuals in the non-match category, \authorcol{13/191} individuals in the inconclusive category, and \authorcol{112/183} individuals in the match category chose to convict. 
...
For the match condition, 117 individuals chose to convict, while 74 individuals chose not to convict, despite the bullet matching being the only evidence against Cole in the crime. 
...
However, in the case of the match condition, those who did not convict gave a generally higher probability that the gun was used in the crime than the probability that Cole committed the crime.
...
probplot <-
  ...
  scale_fill_manual(values = c("grey20", "plum1"))+
  theme_bw()
...
As \@ref(fig:cred) demonstrates, "Extremely Credible" was by far the most selected category for the firearms examiner, with some people selecting "Moderately Credible", while the other categories quickly drop off. 
This trend was also reflected in the data for the algorithm expert, and can be seen in most histograms of this data (leading to a question of scale compression). 
...
In terms of reliability, participants appeared to find the case evidence less reliable when an inconclusive decision was given than they did when a conclusive decision was reached, as shown in \@ref(fig:caserel). 
Note that this question was only answered by participants who received the algorithm condition. 
...
Individuals were also asked to rate the general reliability of firearms evidence as a field (\@ref(fig:genrel)). 
...
This trend is also shown in the non-match condition when the algorithm is present, as both of the highest categories appear to have a similar proportion of respondents. 
However, it is not reflected in the match condition when the algorithm is absent. 
...
In all conditions, participants were asked to rate the reliability of the examiner's subjective opinion of the firearm evidence (\@ref(fig:examrel)).
...
Individuals who received the algorithm were also asked to rate algorithm reliability, \chaircol{responses are shown in} \@ref(fig:algrel). 
...
In general, individuals appeared to give lower reliability ratings when an inconclusive decision was reached. 
...
In terms of firearms evidence as a field, results are shown in \@ref(fig:gensci). 
...
A higher proportion of individuals selected "Extremely scientific" for conclusive decisions when the algorithm was presented compared to when the algorithm was absent.
...
For how scientific individuals found the examiner's comparison, the algorithm only seemed to have an effect when individuals were presented with an inconclusive decision (\@ref(fig:examsci)). 
...
When the algorithm was present, however, "Moderately scientific" became the most popular choice for those who received an inconclusive decision, which reflects the general trend of the majority of participants selecting the second highest category for inconclusive results that was seen when previously discussing reliability and other questions of scientificity. 
...
Participants gave the algorithm a high rating in scientificity across all categories of conclusions, as shown in \@ref(fig:algsci). 
...
For the firearms examiner's personal comparison, few individuals selected that they understood less than half the method (\@ref(fig:expunder)). 
Those who did not receive the algorithm were more likely to select "I understood everything" compared to those who did receive the algorithm, across all conclusion conditions. 
There was not a \chaircol{discernible} difference in responses based on the presence or absence of images.
...
The results for the particpants' understanding of the algorithm description differs based on the presence or absence of images (\@ref(fig:algunder)).
...
Individuals were asked whether or not they thought that guns left unique markings on discharged bullets and casings \authorcol{after reviewing the testimony}. 
Of the 588 responses, only 16 individuals indicated that they did not think guns left unique markings. 
...
Participants were asked to evaluate uniqueness after reading their provided testimony.\chaircol{This sentence is a bit out of place? What is the goal here?}\authorcol{Would responses be different if they were asked to evaluate uniqueness before reading the testimony?}
...
When asked about the case against the defendant, shown in \@ref(fig:strength), there was a small difference in terms of the algorithm. 
...
We compared how individuals scored strength of evidence and the probability that they assigned to Cole committing the crime and the gun being present at the crime scene. 
These results were largely consistent (\@ref(fig:probstrength)). 
...
"Rarely" was by far the most selected category, as shown in \@ref(fig:mistakes). 
There does not appear to be a relationship between how often individuals felt firearms examiners made mistakes and factors such as conclusion, images, or the algorithm.
...
Results were compared both on an individual level as well as on a group level. 
...
In terms of credibility, as shown in \@ref(fig:histcred), the algorithm and the expert scored fairly similarly, with the algorithm expert resulting in slightly more individuals selecting "Extremely credible" than the firearms expert. 
...
For reliability, show in \@ref(fig:histrel), a similar number of individuals selected "Extremely reliable" for both the algorithm evidence and the examiner's comparison. 
...
Individuals generally saw the algorithm as more scientific than the expert, as shown in \@ref(fig:histsci). 
They were more likely to select "Extremely scientific" in the case of the algorithm when compared to the expert. 
Accordingly, they were more likely to select "Moderately scientific" in the case of the examiner when compared to the algorithm. 
...
```
\@ref(fig:coordcred) depicts parallel coordinate plots for participants' ratings of the credibility of the experts, as well as their understanding of the methods described in the testimony. 
...
It can also be seen that there is a trend of individuals selecting a category lower for the algorithm compared to what they selected for the examiner. 
This corresponds to \@ref(fig:histunder) in that individuals tend to give higher understanding ratings to the examiner's bullet comparison method than they did for the algorithm.
...
\@ref(fig:coordscirel) demonstrates the trends for selection for how scientific and reliable individuals felt the evidence was. 
...
Some participants switched between the two highest categories for the algorithm or the examiner, with similar numbers giving higher values to the algorithm and the examiner.  
...
testcred_df2 %>%
  ...
  geom_pcp( alpha = 0.75) +
  scale_colour_manual(values = c("#FF8E00", "grey20")) +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  geom_pcp_boxes(fill = "transparent", color = "black", linewidth = 1)+
#  geom_pcp_labels() +
  facet_grid(conclusion~.)+
  theme_bw()
 # theme(legend.position="top") #aspect.ratio = 0.4, 


```

These coordinate plots indicate that individuals tended to choose the same categories across variables, with most variation between categories resulting from individuals moving up or down a single category. 
...
This corresponds with @garrett2013's study of fingerprint match language, where they did not find a significant difference in the participants' feelings of guilt based on various match conclusions.
...
Participants also found the algorithm to be more scientific than the examiner. 
...
@garrett2013 suggests that feelings of guilt may be reduced if the examiner admits that their conclusion may be in error.
This effect may be diminished by adding a stronger cross examination, with special attention paid to error rate studies, or by asking participants to read critical articles (such as PCAST or information about the CSI effect) prior to reading the provided court testimony.
