...
test_comments %>% filter(page_count==2 | page_count==3) %>%
  ...
  pander(caption = "Participant Note Example", justify = 'left')
...
fuzzy_matches %>% 
  ...
  arrange(dist) %>%
    head(n=10) %>% 
  pander(caption = "Close Fuzzy Matches from the Testimony", justify = 'left')
...
fuzzy_matches %>% 
  ...
  arrange(-dist) %>%
    head(n=5) %>% 
  pander(caption = "Distant Fuzzy Matches from the Testimony", justify = 'left')
