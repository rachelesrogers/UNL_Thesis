...
page2_notes <- study1_notes %>% filter(page_count == 2) %>% 
...
The frequencies for 'willfully' can be averaged: $\frac{193+165+162+78+77}{5}=135$.
...
include_graphics(path = "images/updatedcollocation.jpg")
```
...
include_graphics(path = "images/updatedweightedcollocation.jpg")
```
...
### Final Output
This final note analysis, which includes fuzzy matching, can be found at https://rachelesrogers.github.io/Presentations/Collocations/Collocation%20Comment%20Analysis.html.
...
nonfuzzy_table <- collocation_object_nonfuzzy[order(-collocation_object_nonfuzzy$col_1),] %>% select(col_1, collocation) %>% rename(Nonfuzzy=col_1)
...
total_table <- total_table[,c(2,1,3)]
 ...
 total_table %>%
  head(n=20) %>% kable(caption = "Table of Collocation Frequencies",
                       align = c("l","c","c"))
