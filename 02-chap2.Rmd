...
Other algorithms \svp{discussed in} @parker1989 use code-specific features such as number of operators, lines of comments, and number of various loop statements.
...
There are \svp{many} examples of pattern matching in text analysis - such as @baeza-yates and @landau1988. 
...
Solutions for finding the longest common substring \svp{utilize either} the construction of a suffix tree [@charalampopoulos2021] or table [@landau1988].
...
\svp{even though} @ptxqc warns that this LCS method is inefficient, and meant to be used in small instances.
...
While 'stringdist_join' will give values for all matches between the two data sets, we can \svp{then identify} the month that is closest to the misspelled value.
...
In scenarios such as those shown in Table \@ref(tab:fncmethod), a straightforward method for note cleaning would be to measure the length of the previous page's notes (n), and compare the previous notes to the first n characters in the current pages' notes via edit distance.
The characters that would be compared between the first and second pages are shown in red in the Page 2 column.
...
Collocations are identified in the reference text, and assigned ordered word numbers (so the first collocation of length 5 will have words 1-5, the second will have words 2-6, and so on). 
...
In order to create a highlighting effect in the page notes, we first implemented a graph of the testimony in 'ggplot2' [@ggplot] in order to assign color values to the frequencies based on a continuous gradient.
...
For clarity, when the study transcript corresponds across all scenarios, the frequency for all scenarios are considered together.
...
\svp{Ok, so I think the problem with isolating this as a case study is that the methods above are entirely justified based on the case study. I would prefer to isolate these justifications - it'll make it easier to write this paper with wiki data as well as our data. Do you think it's doable? Maybe even just by using sequential edits/comparisons as the example?} 
...
Table \@ref(tab:noteexample) shows two sequential note pages used in the validation dataset.
In this case, the FNC method could easily be applied - because the text from page 2 perfectly aligns with the beginning of the page 3 text, resulting in an edit distance of 0.
...
For this method comparison study, I changed the threshold with values of 0 character difference, 5 character difference, and 15 character difference (set as less than 1, less than 6, and less than 16).
...
In the validation study, I tried several different character lengths: 40 characters, half the previous note length, a third of the previous note length, and a fourth of the previous note length plus 5 (to compensate for shorter strings).
...
Here, "collocation.x" refers to the collocation found in the transcript, while "collocation.y" refers to the "misspelled" collocation.\svp{I might rename these so that the table is clear and doesn't require explanation.}
...
The "close_freq" column indicates if the participant's collocation matches with multiple collocations from the study - such as "Jur" in the example above.
...
In this case, we also see that the participant collocation of "perception of trial information presentation" was an equal edit distance from two collocations, based on the "close_freq" value.
...
fuzzy_matches %>% 
  ...
  arrange(-dist) %>%
    head(n=5) %>% 
  pander(style = "grid",
         caption = "(\\#tab:fuzzyexamlarge) Distant Fuzzy Matches from the Testimony", justify = 'left', split.cells = 20)
