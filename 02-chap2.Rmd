...
fuzzy_matches$collocation.x <- str_wrap(fuzzy_matches$collocation.x, width=15)
fuzzy_matches$collocation.y <- str_wrap(fuzzy_matches$collocation.y, width=15)
  
fuzzy_matches %>% 
  select(collocation.x, collocation.y, dist, count, close_freq) %>% 
  arrange(dist) %>%
    head(n=10) %>% kable(caption = "Close Fuzzy Matches from the Testimony")
...
fuzzy_matches %>% 
  select(collocation.x, collocation.y, dist, count, close_freq) %>% 
  arrange(-dist) %>%
    head(n=5) %>% kable(caption = "Distant Fuzzy Matches from the Testimony")
